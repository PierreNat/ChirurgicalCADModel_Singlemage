...
result_dir = os.path.join(current_dir, 'results/2_rotation_regression')
...
def main():
    ...
    for i in loop:
        ...
        for image, silhouette, parameter in train_dataloader:
            ...
            loss = nn.MSELoss()(params, parameter[0, 0:3]).to(device)
            ...
            if(i == iterations-1):
                ...
                plt.savefig('results/2_rotation_regression/Final_regression_rotation_{}iterations_{}.png'.format(iterations, file_name_extension),  bbox_inches = 'tight', pad_inches = 0.05)
    ...
    fig.savefig('results/2_rotation_regression/regression_1image_Translation_3params_{}.pdf'.format(file_name_extension), bbox_inches = 'tight', pad_inches = 0.05)
    fig.savefig('results/2_rotation_regression/regression_1image_Translation_3params_{}.png'.format(file_name_extension), bbox_inches = 'tight', pad_inches = 0.05)
    matplotlib2tikz.save("results/2_rotation_regression/regression_1image_Translation_3params_{}.tex".format(file_name_extension),figureheight='5.5cm', figurewidth='15cm')
    plt.show()
